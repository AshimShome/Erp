...
//            dd( $vender); ->orderByDesc("name")
            $vender = Vender::where('created_by', '=', \Auth::user()->creatorId())->orderByDesc("name")->get()->pluck('name', 'id');
//            dd( $vender);
            $vender->prepend('Select Vendor', '');

            $status = Bill::$statues;

            $query = Bill::where('created_by', '=', \Auth::user()->creatorId())->latest();
...
//        $test_data =BillProduct::sum('price')->where('id','bill_id')->get();
//        $data = DB::table("bill_products")->sum('price');
//        dd($data);
        $data =BillProduct::with('venderCount')->sum('price');
//
...
//            dd($customFields)
            ...
            $bankAccount=BankAccount::where('created_by', \Auth::user()->creatorId())->get()->pluck('holder_name', 'id');
            ...
            return view('bill.create', compact('venders','bankAccount', 'bill_number', 'product_services', 'category', 'customFields', 'vendorId'));
        }
        else
        {
...
//                    $billProduct->discount    = isset($products[$i]['discount']) ? $products[$i]['discount'] : 0;
    ...
    public function product(Request $request)
    {
        ...
        $data['unit']        = !empty($product->unit()->first()) ? $product->unit()->first()->name : '';
...
//            . (($resp['is_success'] == false && !empty($resp['error'])) ? '<br> <span class="text-danger">' . $resp['error'] . '</span>' : ''));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
...
//            dd($bill);
            ...
            $categories = ProductServiceCategory::where('type',2)->where('created_by', '=', \Auth::user()->creatorId())->get()->pluck('name', 'id');
