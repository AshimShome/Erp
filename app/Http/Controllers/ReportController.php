...
class ReportController extends Controller
{
    ...
    public function billSummary(Request $request)
    {
        ...
        if (\Auth::user()->can('bill report')) {
            $vendorBills = Vender::with('bill', 'payment')->get();
            $total_payment = Payment::sum('amount');
            $total_payment_bill_price = BillProduct::sum('price');
            $total_payment_bill_quantity = BillProduct::sum('quantity');
            $total_payment_bill = DB::table('bill_products')->sum(\DB::raw('price * quantity'));
            $total_due = $total_payment_bill - $total_payment;
            $filter['vender'] = __('All');
            $filter['status'] = __('All');
...
//            dd(  $totalPaidBill );
            ...
            // dd($vendorBills[0]->bill->bill_id);

            return view('report.bill_report', compact('bills', 'vender', 'status', 'totalBill', 'totalDueBill', 'totalPaidBill', 'billTotal', 'monthList', 'filter', 'vendorBills', 'total_payment_bill', 'total_payment', 'total_due'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied.'));
        }
    }
